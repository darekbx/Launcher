...
int* createColorBufferFlat(jint* positions, int positionsCount, jint* imageData, jint imageSize) {
    int *colorBuffer = malloc(positionsCount*sizeof(int));
    int index = 0;
    for (int i = 0; i < positionsCount; i += 4) {
        ...
        for (int j = 0; j <= 3; j++) {
            colorBuffer[i + j] = extractedColor[j];
        }
    }
    return colorBuffer;
}
...
Java_com_testapplication_NativeBufferCreator_translateColorToHeight(JNIEnv *env,
                                                                    jobject instance,
                                                                    jint color) {
...
Java_com_testapplication_NativeBufferCreator_extractColor(JNIEnv *env,
                                                          jobject instance,
                                                          jint color) {
...
Java_com_testapplication_NativeBufferCreator_randomNoise(JNIEnv *env,
                                                         jobject instance) {
    return randomNoise();
}
...
Java_com_testapplication_NativeBufferCreator_createVertexBufferFlat(JNIEnv *env,
                                                                    jobject instance,
...
Java_com_testapplication_NativeBufferCreator_createColorBufferFlat(JNIEnv *env,
                                                                   jobject instance,
                                                                   jintArray positions_,
                                                                   jintArray imageData_,
                                                                   jint imageSize) {
    ...
    int *colorBuffer = createColorBufferFlat(positions, positionsCount, imageData, imageSize);

    jbyteArray result = (*env)->NewByteArray(env, positionsCount);

    (*env)->ReleaseIntArrayElements(env, positions_, positions, 0);
    (*env)->ReleaseIntArrayElements(env, imageData_, imageData, 0);
    (*env)->SetByteArrayRegion(env, result, 0, positionsCount, colorBuffer);
    return result;
}
